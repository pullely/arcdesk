...
// deleted; all wire I/O now goes through the typed `Arcdesk` resource
...
//   - `createClient` — constructs a `Arcdesk` against a target, wired
...
//     `Promise<T>` + thrown `ArcdeskError` into the same shape the
...
import { Arcdesk, ArcdeskError, type ClientOptions } from "@saas/sdk";
import { apiEdgeWorkersDevUrl } from "./app-config";

export type ApiClient = Arcdesk;
...
export function createClient(target: ApiTarget, token: string | null): Arcdesk {
  const opts: ClientOptions = { baseUrl: target.url };
  if (token) opts.auth = { kind: "bearer", token };
  return new Arcdesk(opts);
}

/**
 * Adapt an SDK promise into the `ApiResult<T>` envelope. Typed
 * `ArcdeskError` subclasses surface as `{ ok: false }` carrying the
