...
Your backend holds one Arcdesk API key and **zero GitHub credentials**.
...
const arcdesk = new Arcdesk({
  baseUrl: process.env.ARCDESK_API_URL!,
  token: process.env.ARCDESK_API_KEY!, // service-principal key
});

// Mint a token scoped to exactly the repos + permissions you need.
// TTL ≤ 1h; mint per task and let it expire — never store it.
const { token } = await arcdesk.integrations.issueGithubToken(ORG_ID, {
