...
/** The actor recorded for anything a homeowner does through the public lane. */
export const HOMEOWNER_ACTOR: AuditActor = { type: "system", id: "arc-public-form" };
...
export interface AuditInput {
  ...
  actor: AuditActor;
  requestId: string;
  subjectKind: "arc_request" | "arc_board";
  subjectId: string;
  subjectName: string;
  description: string;
 ...
 *
 * Deliberately NOT `EventsRepository.appendEventWithAudit`: that method is one
 * Postgres statement (`WITH … INSERT … RETURNING`, `row_to_json`, `FULL JOIN`)
 * inherited from the Postgres baseline, and SQLite — so D1 — rejects DML
 * inside a CTE. It fails on every call. Here the event goes in through
...
export async function recordAudit(executor: SqlExecutor, input: AuditInput): Promise<boolean> {
  ...
  try {
    ...
    if (!appended.ok) {
      warn("arc event append failed", input);
      return false;
    }
    await executor.execute(
      `INSERT INTO events_audit_entries
       ...
       SELECT $2, id, org_id, project_id, environment_id, actor_type, actor_id,
              ...
              'arc', $3, occurred_at, request_id, correlation_id, payload, redact_paths
         FROM events_event_log WHERE id = $1`,
      [eventId, crypto.randomUUID(), input.description],
    );
    return true;
  } catch {
    warn("arc audit append threw", input);
    return false;
  }
}
