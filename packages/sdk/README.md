...
Arcdesk TypeScript SDK — typed, dependency-free, runtime-agnostic client
for the Arcdesk control plane API.

## Install (workspace)

This package is internal to the `arcdesk` monorepo. Add it as a
...
```ts
import { Arcdesk } from "@saas/sdk";

const client = new Arcdesk({
  baseUrl: "https://api.arcdesk.app",
  auth: { kind: "bearer", token: process.env["ARCDESK_TOKEN"]! },
});
...
All non-2xx responses throw a typed subclass of `ArcdeskError`. Branch on
...
| `ArcdeskError` (base)   | _any_                 | _any_          |

Unknown error codes (forward-compatible — e.g. a future `quota_exceeded`)
decode to the base `ArcdeskError` carrying the raw envelope. Non-JSON 5xx
...
A custom `fetch` implementation can be injected via `new Arcdesk({ fetch })`.
