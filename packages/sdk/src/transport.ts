// Base HTTP transport for the Arcdesk SDK.
//
// Goals:
...
// - Typed errors: every non-2xx response is decoded into a `ArcdeskError`
...
export interface ClientOptions {
  /**
   * Base URL of the Arcdesk api-edge worker, e.g.
   * `https://api.arcdesk.app`. Trailing slash is stripped.
   */
  baseUrl: string;
 ...
 * `Arcdesk` (the client class wired up in `index.ts`).
 */
export class Transport {
  ...
  constructor(options: ClientOptions) {
    ...
    if (typeof fetchImpl !== "function") {
      throw new TypeError(
        "Arcdesk SDK requires a fetch implementation. Pass `fetch` in options on platforms without a global.",
      );
    }
...
export function generateRequestId(): string {
  ...
  throw new Error(
    "Arcdesk SDK requires the Web Crypto API (globalThis.crypto). No source of randomness was found.",
  );
}
