...
describe("Transport — success path", () => {
  it("unwraps the data envelope on GET", async () => {
    ...
    const client = new Arcdesk({ baseUrl: "https://api.test", fetch });
...
describe("Error decoding — typed branches", () => {
  ...
  const cases: Array<[string, number, new (...args: never[]) => ArcdeskError]> = [
  ...
  it("forward-compat: unknown error code → base ArcdeskError", async () => {
    ...
    try {
      ...
      expect(err).toBeInstanceOf(ArcdeskError);
      expect(err).not.toBeInstanceOf(BadRequestError);
      expect((err as ArcdeskError).code).toBe("future_quota_exceeded");
    }
  });
