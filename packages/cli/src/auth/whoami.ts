// `arcdesk whoami` — read token + cached context, validate via SDK.
...
export interface WhoamiInput {
  ...
  /** SDK factory override for tests. Defaults to constructing `Arcdesk`. */
  readonly sdkFactory?: (baseUrl: string, token: string) => Arcdesk;
}
