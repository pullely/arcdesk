...
describe("resolveConfigDir", () => {
  it("respects ARCDESK_CONFIG_DIR override", () => {
    const prev = process.env["ARCDESK_CONFIG_DIR"];
    process.env["ARCDESK_CONFIG_DIR"] = "/tmp/cli-override";
    try {
      expect(resolveConfigDir()).toBe("/tmp/cli-override");
    } finally {
      if (prev === undefined) delete process.env["ARCDESK_CONFIG_DIR"];
      else process.env["ARCDESK_CONFIG_DIR"] = prev;
    }
  });
});
