// CLI error helpers. Translate `ArcdeskError` subclasses (from
...
export class MissingOrgContextError extends Error {
  constructor() {
    super("no active organization (run `arcdesk org use <id>`)");
...
export function formatCliError({ err, mode }: FormatErrorInput): FormattedError {
  ...
  if (err instanceof UnauthenticatedError) {
    return formatSdk(4, err, "token rejected — run `arcdesk login` to refresh", mode);
  }
  if (err instanceof ArcdeskError) {
