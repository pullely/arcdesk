...
`arcdesk` — first-class TypeScript CLI for the Arcdesk control
...
This package is internal to the `arcdesk` monorepo and is not
...
```
arcdesk login    [--api-url=URL] [--token=BEARER]
arcdesk logout
arcdesk whoami
```
...
```
arcdesk org list
arcdesk org use <org-id>
arcdesk org members
arcdesk project list
```

Writes (Task 0101):

```
arcdesk org invite <email> [--role=ROLE] [--idempotency-key=KEY] [--org=ORG_ID]
arcdesk project create <name> [--idempotency-key=KEY]
arcdesk env create <project-id> <name> [--idempotency-key=KEY]
arcdesk api-key create <name> [--scope=SCOPE] [--idempotency-key=KEY]
arcdesk webhook create <url> [--event=EVENT[,EVENT2,...]] [--idempotency-key=KEY]
```

Cross-resource reads (Task 0101):

```
arcdesk usage summary    [--metric=METRIC] [--from=ISO] [--to=ISO]
arcdesk billing summary
arcdesk audit list       [--limit=N] [--cursor=CURSOR] [--category=CAT] [--all]
```
...
context (`arcdesk org use <org-id>`). Only `org invite` accepts an
...
The shipped V1 is **token-paste**: `arcdesk login` prompts for a
...
- `FileTokenStore` fallback: `~/.config/arcdesk/credentials.json`,
  mode **0600**, parent directory mode **0700**.

Active organization context lives at
`~/.config/arcdesk/config.json` (mode 0644, not a secret). Override
both via `ARCDESK_CONFIG_DIR` (used by tests).
